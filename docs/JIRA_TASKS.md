...
- [ ] CPU, 메모리, 디스크 요구사항 확인
- [ ] Yocto/BitBake를 일반 사용자로 실행
- [ ] Python `pexpect`, loop device, mount 권한 확인
...
- [ ] Poky `yocto-6.0.2` 준비
- [ ] `meta-raspberrypi` `wrynose` 준비
- [ ] 소스 버전과 commit 출력
- [ ] 기존 소스 재사용 시 불필요한 clone 방지

완료 조건:

- `setup-yocto.sh` 한 번으로 호환 레이어가 준비됨
- Poky와 BSP 레이어가 동일한 Wrynose 계열임

### BSP-03 RPi4 이미지 호스트 빌드

- [ ] `MACHINE=raspberrypi4-64` 설정
- [ ] `core-image-base` 빌드
- [ ] 다운로드/sstate 캐시 경로 고정
- [ ] WIC, 커널 Image, BCM2711 DTB 산출물 확인
- [ ] 자동 테스트용 serial console 및 root login 설정
...
- [ ] 별도 `meta-device-simulation` 레이어 생성
- [ ] virtio-blk, virtio-pci, 9p 설정 built-in 적용
- [ ] 특정 커널 버전에 종속되지 않는 bbappend 적용
- [ ] 최종 커널 config 검증
...
- [ ] WIC 크기를 QEMU SD 제약에 맞게 조정
- [ ] rootfs 파티션을 동적으로 탐색
- [ ] 테스트 파일을 WIC에 주입
- [ ] BCM2711 DTB와 `raspi4b` 머신으로 부팅
- [ ] MMC rootfs, UART, Device Tree, SoC 노드 확인
- [ ] 테스트 결과를 WIC에서 회수

완료 조건:

- QEMU `raspi4b`에서 root 로그인이 성공함
...
- [ ] 전체 재현 명령 README 반영
- [ ] 프로덕션 이미지에서 `debug-tweaks` 제거 조건 명시
...
- T3 WIC 주입에는 호스트 `sudo`와 loop/mount 권한이 필요
