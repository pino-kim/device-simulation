...
# 사용법: bash /data/yocto-rpi4/poc/run-poc.sh
set -e
IMG=qemu-runner:latest   # qemu 8.2 + 런타임 라이브러리가 든 이미지
docker rm -f poc-run 2>/dev/null || true
rm -f /data/yocto-rpi4/poc/testshare/result.txt
...
docker run --rm --name poc-run --security-opt seccomp=unconfined \
  -v /data/yocto-rpi4:/workdir -w /workdir \
  $IMG expect -f /workdir/poc/drive.expect

echo
echo "===== 회수된 테스트 결과 (poc/testshare/result.txt) ====="
cat /data/yocto-rpi4/poc/testshare/result.txt 2>/dev/null || echo "(결과 파일 없음 — 부팅/테스트 로그 확인 필요)"
